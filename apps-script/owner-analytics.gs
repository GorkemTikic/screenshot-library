...
function _ensureOwnerAgg_(agg, owner, ownerKey) {
  ...
  if (!agg[key]) agg[key] = {
    owner: owner, ownerKey: key, owned: 0, lifetimeIds: {}, total: 0,
...
function rebuildOwnerStats() {
  ...
  if (values.length >= 2) {
    ...
    for (var rowIndex = 1; rowIndex < values.length; rowIndex++) {
      ...
      var detail = ownerAgg.titles[record.id] || (ownerAgg.titles[record.id] = {
        id: record.id, title: record.title, total: 0, copies: 0, responseCopies: 0,
        imageCopies: 0, editedImageCopies: 0, views: 0, agents: {}, last: ''
      });
      detail.total++;
