...
const records = [
  ...
  { id: 4, title: 'Legacy app guide', text: 'Legacy copy', owner: 'CS Gorkem T', topic: 'General', language: 'English' },
];

test('replacement filtering searches every identifying field and excludes archived records', () => {
  for (const query of ['funding', 'enzo', 'futures', 'chinese']) {
...
test('legacy records search by their displayed normalized mobile platform', () => {
  assert.deepEqual(filterStudioItems(records, { query: 'mobile' }).map((item) => item.id), [1, 4]);
  assert.deepEqual(filterStudioItems(records, { platform: 'mobile' }).map((item) => item.id), [1, 4]);
});
